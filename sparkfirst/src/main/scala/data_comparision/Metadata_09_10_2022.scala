...
object Metadata_09_10_2022 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val MAINdf1 = spark.sql("""select upper(tablename) as TABLE_NAME,upper(columnname) as COLUMN_NAME,upper(tabletype) as TABLE_TYPE from  MAIN""")
    MAINdf1.show(100)
    ...
    df2.createOrReplaceTempView("DASP")
    val Snowdf2 = spark.sql("""select upper(tablename) as TABLE_NAME,upper(columnname) as COLUMN_NAME, upper(tabletype) as TABLE_TYPE from  DASP """)
    println("====Sort by tablename=====")
